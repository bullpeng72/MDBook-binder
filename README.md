...
- **마크다운 표는 행의 셀 단위로 번역한다** — 표 청크 전체를 한 번에
  ...
  모델에 보내고 표 구조 자체는 코드가 그대로 보존한다(구분선 행과 번역할
  원본 언어 문자가 없는 셀 — k2e는 한글, e2k는 영문 — 은 모델을 거치지
  않는다).
- **청크마다 번역 결과가 완전한지 자동 검증하고 필요하면 재시도한다**
  (k2e·e2k 둘 다) — 자세한 동작은 아래 [번역 완전성
...
- **`--resume`으로 중단 후 이어할 수 있다** — `OUT_DIR`에 이미 결과
  ...
  다시 돌리지 않아도 된다. **단, 재시도 후에도 청크가 미완료였던
  챕터(k2e는 한글 잔존, e2k는 원문 그대로 에코된 청크)는 `OUT_DIR`에
  `<파일명>.incomplete.json` 마커가 남아있어 "이미 있음"으로 건너뛰지
  않고, 그 마커에 캐시된 청크별 번역 결과 중 실패했던 청크만 다시
  번역한다(청크 단위 이어하기 — 이미 통과한 청크는 다시 모델에 묻지
  않아 더 빠르고, 통과한 청크가 재시도 중 비결정성으로 새로 실패하는
  일도 없다)** — 같은 코퍼스에 `--resume`을 반복 실행할수록(LLM
  출력이 비결정적이므로) 미완료 청크 수가 점차 줄어드는 방향으로
  수렴한다.
...
방향마다 청크 검증 신호가 다르다 — 같은 잣대를 양쪽에 그대로 쓸 수 없기
때문이다.
...
**`e2k`(영→한)**: 위와 대칭인 "영문 비율" 검증은 쓸 수 없다 — 한국어
기술 문서에는 영문 고유명사·약어가 정상적으로 섞이는 게 흔해서, "번역
실패로 영어가 그대로 남은 것"과 "정상적으로 영문 용어가 남은 것"을
신뢰성 있게 구분할 신호가 없기 때문이다. 대신 **원문과 번역 결과가
완전히 동일한 청크**(라틴 문자 20자 이상인 청크에 한해)를 "번역 시도조차
안 함"으로 보고 최대 2회 재시도한다 — 조금이라도 번역이 일어났다면
자연어 산문이 원문과 글자 단위로 완전히 같을 일은 사실상 없다는, 방향과
무관하게 안전한 신호다. 브랜드명 한 단어짜리 청크처럼 원래도 안 바뀌는
게 정상인 짧은 청크(라틴 문자 20자 미만)는 오탐하지 않는다.
...
- **`build pdf`는 챕터 간 상호참조 링크를 앵커로 연결하지 않는다**: `build
  html`은 `[§5](../Chapter_05.md)` 같은 링크를 같은 HTML 안의 `#앵커`로
  재작성해 실제로 클릭 가능하게 만드는데(위 상호참조 섹션 참고), PDF는
  챕터마다 별개로 렌더링한 뒤 pypdf로 병합하는 구조라(`--merge` 포함) 한
  챕터의 렌더링 시점엔 다른 챕터의 앵커 대상이 아직 존재하지 않는다.
  존재하지 않는 `.md` 경로를 가리키는 죽은 링크로 남기는 대신 링크
  껍데기만 벗기고 텍스트는 그대로 보존한다 — PDF에서는 "5장을 참고"처럼
  일반 텍스트로만 보이고 클릭은 안 된다.
...
### 0.5.6 (2026-08-22) — e2k 표 번역 방향 버그 + edit/build/import 정독 버그 10건

- **fix**: e2k(영→한) 방향에서 표 셀이 전혀 번역되지 않던 버그 — 셀
  스킵 판정이 "한글 유무"로 고정돼 있어 원문이 영어인 e2k 표는 모든 셀이
  스킵됐다. 방향별로 반대 신호(k2e는 한글, e2k는 라틴 문자)를 쓰도록 수정
- **feat**: e2k도 k2e처럼 청크 검증·재시도·`.incomplete.json` 마커·
  `--resume` 이어하기를 지원 — 원문과 결과가 완전히 동일한 청크를 "번역
  시도조차 안 함"으로 판정(`is_untranslated_echo`)
- **fix**: `edit`에서 저작 마크다운의 일반 이미지가 섹션을 열었다가
  저장만 해도(no-op) 통째로 사라지던 버그(가장 심각)
- **fix**: `edit`에서 다이어그램·콜아웃이 저장할 때마다 섹션 맨 끝으로
  밀리던 버그, 및 그 수정이 만든 회귀(인접한 보존 요소 2개 이상이면
  순서가 뒤바뀜)까지 재검증 과정에서 발견해 함께 수정
- **fix**: `edit` 섹션 삭제 시 다른 챕터에 남는 죽은 상호참조 링크,
  동일 src 이미지가 여러 개일 때 삭제/교체 요청이 조용히 무시되던 문제
- **fix**: `import`가 표 셀의 `|` 문자를 이스케이프하지 않아 열이
  어긋나던 문제, 페이지를 넘어가는 표의 헤더 행이 러닝 헤더로 오인돼
  지워지던 문제
- **fix**: `build`에서 blockquote 안 Mermaid 다이어그램이 깨지던 문제
  (정규식 줄 시작 앵커링 누락)
- **fix**: `build pdf`가 챕터 간 상호참조 링크를 죽은 `.md` 링크로
  남기던 문제(텍스트로 unwrap) 및 누락된 이미지를 경고 없이 조용히
  빈 자리로 렌더링하던 문제(html_book.py와 동일하게 요약 경고)
- 🧹 **techdebt**: `html_book.py`/`pdf_book.py`에 각각 따로 구현돼 있던
  파일명 중복 방지 로직을 `manifest.dedupe_suffix()`로 통합, 공유 유틸
  `imgembed.py` 직접 테스트 신설
- `tests/test_render.py`·`tests/test_imgembed.py` 신설, 회귀 테스트
  60건 추가(총 388개)
