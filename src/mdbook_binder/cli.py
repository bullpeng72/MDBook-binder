...
def translate_cmd(
    ...
    다시 실행해 이어서 진행할 수 있다. 재시도 후에도 청크가 미완료였던
    챕터(k2e는 한글 잔존, e2k는 원문 그대로 에코된 청크)는 "이미 있음"으로
    건너뛰지 않고, 마커에 캐시된 청크별 번역 결과 중 실패했던 청크만 다시
    번역한다(청크 단위 이어하기 — 이미 통과한 청크는 다시 모델에 묻지
    않는다). 같은 코퍼스를 `--resume`으로 반복 실행할수록 미완료 청크가
    점차 줄어드는 방향으로 품질이 개선된다.
    """
